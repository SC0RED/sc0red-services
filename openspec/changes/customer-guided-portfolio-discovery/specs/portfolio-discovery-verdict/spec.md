...
Portfolio discovery SHALL return a structured verdict alongside the candidate list, containing: the method that produced the result, the candidate count, a completeness signal (one of: full site list, client-side-rendered, site blocked, web-search subset, genuinely empty), and the set of next actions available to the customer (a subset of: search deeper, render site, upload list). The verdict SHALL be derived from existing discovery signals (which path produced candidates, whether the site fetch failed, whether the listing page was an empty/client-side shell).
...
- **THEN** the verdict reports method=web-search, the subset count, completeness=client-side-rendered, and next actions including search-deeper and upload-list
...
- **WHEN** a verdict indicates a client-side-rendered or blocked site with a partial count
