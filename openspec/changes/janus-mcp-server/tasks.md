...
- [x] 2.5.15 Bug C — **FIXED.** Confirmed live after the LWA deploy: OAuth metadata advertised `issuer`/`authorize`/`token`/etc. at `mcp.staging.sc0red-services.sc0red.com` (non-existent DNS) because `MCP_ISSUER_URL` was never set → handler used the bogus default. Fix: `MCPConstruct` now sets `MCP_ISSUER_URL = function_url.url` (set after `add_function_url` since the URL derives from the function). Also cleaned up the handler's misleading non-existent-DNS defaults → localhost (local-dev fallback).
