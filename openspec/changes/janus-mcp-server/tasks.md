...
- [ ] 2.5.15 Bug C — **REOPENED. First fix attempt (#380) failed at deploy with a CloudFormation circular dependency.** `add_environment("MCP_ISSUER_URL", function_url.url)` creates Lambda env → FunctionUrl → Lambda. The circular line was reverted (`fix/mcp-revert-circular-issuer-env`) to unblock development's deploy pipeline; the handler localhost-default cleanup from #380 was kept. **Durable options (choose one):** (a) **SSM indirection** — store `function_url.url` in an SSM parameter; the Lambda's env carries only the static parameter NAME (no cycle), and the handler reads SSM at module load to set the issuer. IAM grant must use a constructed string ARN, not `param.grant_read`, to avoid re-introducing the cycle via the role→param→url→lambda path. (b) **Custom domain** for the MCP server (issuer known at synth, no cycle) — bigger infra lift (Route53 + cert). Manual CLI env var is NOT viable: CFN resets the Lambda env on the next deploy. Until fixed, OAuth metadata advertises the localhost default → the mcp-inspector OAuth round-trip can't complete (run-once fix from #379 still stands; server boots + serves metadata).
