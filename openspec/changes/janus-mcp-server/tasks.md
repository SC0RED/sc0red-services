...
- 🔴 **Bug K — `/token` 500 on exchange (current blocker).** With CORS fixed the inspector reached the token exchange, which 500'd: `AttributeError: 'StoredAuthorizationCode' object has no attribute 'expires_at'` at `mcp/server/auth/handlers/token.py:145` (`if auth_code.expires_at < time.time()`). Our `Stored*` dataclasses (auth code / refresh / access) omitted `expires_at`, which the SDK token handler + bearer-auth backend read. Never caught because this path was first exercised only now. **Fix in progress** (`fix/mcp-token-expires-at`): add + populate `expires_at` on all three from the stored `ttl` / JWT `exp`.
...
The MCP runtime is healthy and the OAuth dance now reaches the token exchange (Bugs G + I + J fixed; CORS verified live). The remaining blocker is **Bug K** (`/token` 500 — missing `expires_at`). Tackle in this order:
1. **Bug K** — ship `fix/mcp-token-expires-at`. **Requires an MCP Lambda redeploy** (CDK). Then re-run the inspector round-trip → should complete (tokens issued → authed `/mcp` connects → tools list) → close PR 2.5's end-to-end validation.
