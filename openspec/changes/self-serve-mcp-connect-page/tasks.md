...
- [x] 2.1 Add the Connect page/route under `settings/` (grow the existing `connected-apps` page into it; update the Settings nav label/link to "Connect").
- [x] 2.2 Server-address sub-component: shows `mcpServerUrl` with a Copy button and the "sign in with sc0red when prompted" note. Keep under the 360-line component limit.
- [x] 2.3 Tests: renders the configured address; Copy places the exact URL on the clipboard; unavailable-address state.

## 3. Per-client setup instructions (tabs)

- [x] 3.1 Client-tabs sub-component with Claude Desktop / Cursor & others / ChatGPT.
- [x] 3.2 Claude Desktop tab: Connectors-UI + OAuth walkthrough; optional `mcp-remote` bridge `claude_desktop_config.json` snippet (Node 18+) with a Copy button.
- [x] 3.3 Cursor tab: copy-able `~/.cursor/mcp.json` remote-MCP snippet templated on `mcpServerUrl` + OAuth-on-first-use note.
- [x] 3.4 ChatGPT tab: hedged one-liner (beta, Plus/Pro, Developer Mode, add the address, sign in) + link to OpenAI's Developer Mode guide. No reproduced click-steps.
- [x] 3.5 Tests: each tab renders; snippets interpolate the current `mcpServerUrl`; Copy works; ChatGPT entry links out and shows the beta caveat.

## 4. Fold in connected-apps management

- [x] 4.1 Reuse the existing connected-apps list + Disconnect as the page's "Connected apps" section (preserve current consent/disconnect behavior).
- [x] 4.2 Empty state directs the customer to the setup instructions above.
- [x] 4.3 Remove the old bare "How to connect" blurb (superseded by the tabs) and update any links pointing at the old page.
- [x] 4.4 Tests: list + disconnect still work in the new placement; empty state renders.
