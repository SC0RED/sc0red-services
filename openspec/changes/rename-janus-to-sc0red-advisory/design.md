...
The product was built and shipped as **Janus** — codename inherited from the early prototype, retained through the polished-pdf-export and improve-pdf-export-content launches. Leadership has now decided to publicly position the product as **sc0red Advisory**, framed as an AI-augmented advisory companion for mid-market PE buyers and operators. The rename + reorder accompanies a broader sc0red company narrative ("AI tools + advisory for PE") visible on the company website (`https://sc0red.github.io/website-redux/`), which lists sc0red Advisory alongside the separate **sc0red Platform** (an investor-focused product, different codebase).
...
- The customer-facing product is unambiguously called *sc0red Advisory* across UI text, page titles, email templates, alt text, and footer copy. (Phase 1)
- Customer-facing host is `advisory.sc0red.com` (and per-environment variants); old Janus hosts continue to serve via redirect for at least 90 days post-launch. (Phase 2)
...
**Implementation note**: For the OpenSpec workflow, the simplest path is to keep ONE change folder (`rename-janus-to-sc0red-advisory/`) with three phase-tagged task groups in `tasks.md`. Each phase ships as a separate PR but all share one proposal/design until archive. If a phase's scope balloons during apply, it can split into its own OpenSpec change at that point.

### 1. Customer-visible only — internal `janus-*` resource names stay

**Decision**: Customer-visible surfaces (URL, brand text, logo, page titles, emails) flip to sc0red Advisory. AWS resources, CDK stack names, internal package names, repository name, and Docker containers keep `janus-*` everywhere they exist today.
...
**Trade-off**: A new dev opening the codebase will see "janus" everywhere internally and "sc0red Advisory" externally. This is a real onboarding wrinkle. Mitigation: Phase 1 task adds a clear note in `CLAUDE.md` and `README.md` ("sc0red Advisory is the customer-facing product name; resources are named `janus-*` for historical reasons — this is intentional and not a TODO").
...
**Decision**: For at least 90 days after sc0red Advisory launches (Phase 2), `dev.janus.sc0red.com` (and equivalents) returns HTTP 301 redirects to `advisory.sc0red.com` for the matching path. After 90 days, the old host can be decommissioned; remove the Route 53 record.
...
The sub-subdomain pattern keeps environments visually distinct (a developer landing on `dev.advisory.sc0red.com` knows they're not on prod) and matches the existing convention (`dev.janus.sc0red.com`).

**Alternatives considered:**

- `vector-dev.sc0red.com` / `vector-testing.sc0red.com` / `advisory.sc0red.com` — flat at the same level. Less convention-following; production looks like the parent of the others, which is misleading.
- Promotion-based aliasing (`advisory.sc0red.com` always points to the latest stable) — too clever, breaks intuition for non-prod testing.

**Trade-off**: ACM cert needs to cover three hostnames or a wildcard `*.advisory.sc0red.com`. Minor cost. Recommend wildcard.

### 4. Brand text strategy — search + replace, with curation

**Decision**: Treat the rename as primarily a string substitution (`Janus` → `sc0red Advisory` in customer-visible contexts), but explicitly curate the high-stakes copy: page titles, login / signup brand block, marketing landing page tagline, email templates. Bulk replacement of incidental mentions; hand-curate the headline strings.
...
**Decision**: `frontend/public/janus-logo.png` is replaced by a `sc0red-advisory-logo.png` (or `vector-logo.svg` if we get a vector asset from design). Five components reference the file via `<img src="/janus-logo.png" alt="Janus">` — all are updated to the new path + alt text. The old PNG can be deleted post-cutover.
...
- **[Phase 1+2 Risk]** Old host still serves the new code with the new brand, creating a confusing duplicate during the dual-serve window.
  → **Mitigation**: Phase 2 cutover plan — Amplify Janus host is set to 301 redirect to sc0red Advisory host *before* the Phase 1 brand goes live on the new host, eliminating the duplicate-serving window.
- **[Phase 2 Risk]** SEO impact from URL change. Search engines need time to re-index `advisory.sc0red.com`; internal links from social / docs may still hit the old URL.
  → **Mitigation**: 301 redirects preserve PageRank. Update internal docs, GitHub README, the company-site link from sc0red.com → sc0red Advisory app. Submit an updated sitemap to Google Search Console.
- **[Cross-phase risk]** Internal cognitive friction — devs have to remember "we say sc0red Advisory publicly but the code says janus." Onboarding cost.
...
- **[Phase 1 Risk]** Brand assets are missing — we don't yet have a finalised sc0red Advisory logo file.
...
Phase 1 — sc0red-advisory-brand-text
  Step 1.1: Land brand-text PR on `development`. App still serves
            from dev.janus.sc0red.com but the page renders sc0red Advisory
...
Phase 2 — sc0red-advisory-host-cutover
  Step 2.1: Add sc0red Advisory Amplify domains + ACM cert + Cognito
            redirect URI allowlist (both old and new). Land on
            `development`. sc0red Advisory host now serves alongside Janus
            host; both work.
  Step 2.2: Verify on dev.advisory.sc0red.com that login + scan +
            analysis + PDF export all work. dev.janus.sc0red.com
            keeps serving.
  Step 2.3: Cutover gate — switch the OLD dev.janus.sc0red.com
            Amplify domain to 301-redirect to dev.advisory.sc0red.com.
...
showing the new brand when its domain redirects to the sc0red Advisory URL
...
- Phase 2: swap the Amplify domain config back so `dev.janus.sc0red.com` serves the codebase directly. Revert Cognito redirect URI removal (keep both allowlists). sc0red Advisory host stays available for users who already migrated.
- Phase 3: `git revert <PR>` on the reorder commit. Page reverts to the post-PR-#250 order.

## Open Questions (resolved + deferred)

The proposal-level "Open Questions for Leadership" section in `proposal.md` is the canonical list. Phase 1 is now unblocked; Phase 2/3 OQs remain deferred. Summarized here for design-doc context:

| # | Question | Resolution | Phase blocked |
|---|---|---|---|
| 1 | Final product name | ✅ **sc0red Advisory** (Christopher Creel, 2026-05-19) | — |
| 2 | Tagline | ✅ Keep current placeholder "AI Risk & Strategic Intelligence" | — |
| 3 | Logo asset | ✅ Extract from development.sc0red.com | — |
| 4 | Cognito split-brain acceptable? | 🟡 Deferred to Phase 2; mitigated for demo since NextAuth + `USER_PASSWORD_AUTH` keeps auth UI in our React app, not Cognito hosted | Phase 2 |
| 5 | Repo rename | 🟡 Deferred — out of scope | — |
| 6 | Per-env hostname pattern | 🟡 Deferred to Phase 2 | Phase 2 |
| 7 | Email sender display name | ✅ Keep "Janus by SignalField" as-is — no SES / DKIM revalidation needed | — |
| 8 | CSS palette source | ✅ development.sc0red.com is the visual reference (token-level visual audit) | — |
| 9 | Section reorder direction | 🟡 Deferred — Phase 3 is independent | Phase 3 |
