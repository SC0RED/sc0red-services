...
- [x] 1.1.1 In ``backend/src/models/model_company.py``, add a ``PdfExportStatus`` ``str, Enum`` with values ``RENDERING``, ``READY``, ``FAILED``.
- [x] 1.1.2 Add a ``PdfExportRecord(BaseModel)`` with fields: ``status: PdfExportStatus``, ``s3_key: str``, ``started_at: datetime``, ``generated_at: datetime | None``, ``error: str | None``.
- [x] 1.1.3 Add an optional ``pdf_export: PdfExportRecord | None`` field on the ``Analysis`` model (the assessment record).
- [x] 1.1.4 ``CompanyAccessor`` helpers: ``set_pdf_export(...)``, ``get_pdf_export()``, ``clear_pdf_export()``. Mirror the existing ``set_strategy_map`` / ``clear_strategy_map`` shape.

### 1.2 S3 bucket + IAM

- [x] 1.2.1 In ``infrastructure/stacks/pdf_render_construct.py``, define an S3 bucket ``janus-<env>-pdf-exports`` with SSE-S3, block-all-public-access, bucket-owner-enforced object ownership, no lifecycle policy.
- [x] 1.2.2 Grant the PDF Lambda ``s3:PutObject`` on the bucket. Pass the bucket name as the env var ``PDF_EXPORTS_BUCKET``.
- [x] 1.2.3 Grant the API Lambda ``s3:GetObject`` on the bucket. Pass the bucket name to the API Lambda as ``PDF_EXPORTS_BUCKET``.
- [x] 1.2.4 ``cdk synth`` cleanly under each environment config (dev / testing / production).

### 1.3 New backend handlers

- [x] 1.3.1 New file ``backend/src/handlers/pdf_export_handlers.py``. Wire two routes:
  - ``POST /api/export/pdf/<analysisId>`` → ``handle_post_export(...)``
  - ``GET  /api/export/pdf/<analysisId>/status`` → ``handle_get_export_status(...)``
- [x] 1.3.2 ``handle_post_export``:
...
- [x] 1.3.3 ``handle_get_export_status``:
...
- [x] 1.3.4 Stale-detection constant ``RENDERING_STALE_AFTER_SECONDS = 60`` lives next to the handler module.
- [x] 1.3.5 New module ``backend/src/handlers/pdf_presigned_url.py`` (or co-located helper): ``mint_pdf_url(s3_key, expires_seconds=60) -> str``. Uses module-level boto3 S3 client (cache across Lambda warm-restart).

### 1.4 Re-analyse invalidation

- [x] 1.4.1 In the re-analyse handler (locate via ``grep -rn "clear_strategy_map" backend/src``), extend the existing clear-pre-pipeline-state logic:
  - Read ``analysis.pdf_export``.
  - If present + ``s3_key`` set, attempt ``S3.delete_object`` (best-effort; log but don't fail on error).
  - Call ``accessor.clear_pdf_export()``.
- [x] 1.4.2 Tests for the re-analyse handler's new invalidation logic (mock S3 delete + assert DynamoDB clear).

### 1.5 Backend tests (Phase 1)

- [x] 1.5.1 ``test_pdf_export_handlers.py`` (new file). Cases:
...
- [x] 1.5.2 ``test_reanalyse_handler.py`` extension (or existing test file): verify S3 delete + ``pdf_export`` clear happen on re-analyse, S3 failure doesn't fail re-analyse.
- [x] 1.5.3 Full suite: ``uv run pytest tests/ -q`` green, coverage ≥ 95%.
...
- [x] 2.1.2 After rendering the PDF:
...
- [x] 2.1.3 On any thrown exception:
...
- [x] 2.2.1 In ``pdf_render_construct.py``, grant the PDF Lambda:
  - ``s3:PutObject`` on the bucket (Phase 1.2.2).
  - ``dynamodb:UpdateItem`` on the assessment table.
- [x] 2.2.2 Add an SQS DLQ for the PDF Lambda's async-invoke failures. Lambda async config: ``maxEventAge`` reasonable (e.g., 5 min — beyond this, the user has clicked again anyway), ``onFailure`` → DLQ.
- [x] 2.2.3 CloudWatch alarm on DLQ ``ApproximateNumberOfMessagesVisible > 0``.

### 2.3 Python proxy switch

- [x] 2.3.1 ``backend/src/handlers/pdf_render_handlers.py`` (or new handler): the new async-invoke path passes ``InvocationType="Event"``. The synchronous GET path (existing) keeps ``RequestResponse`` for Phase A backward compat.
- [x] 2.3.2 Add ``started_at`` to the async payload so the Lambda can match it in the conditional UpdateItem.

### 2.4 Lambda tests (Phase 2)

- [x] 2.4.1 ``backend/lambdas/pdf-render/tests/handler.test.ts``:
...
- [x] 2.4.2 Integration test that exercises the full path with localstack or moto (S3 + DynamoDB).

### 2.5 Ship Phase 1 + 2 together

- [x] 2.5.1 Combined PR: backend skeleton (Phase 1) + Lambda async owner (Phase 2). They're tightly coupled (the new POST endpoint needs the Lambda to write the result back).
- [x] 2.5.2 Architecture-reviewer pass on the diff.
- [x] 2.5.3 Ship through dev → testing → production. **The synchronous GET path is still active**, so frontend behaviour is unchanged at this stage.
- [x] 2.5.4 Smoke test on dev: hit the new POST endpoint directly (curl), verify the async-invoke fires, S3 object appears, assessment record updates.

## 3. Phase 3 — Frontend migration

### 3.1 ExportPDFButton becomes status-aware

- [x] 3.1.1 In ``frontend/src/components/ExportPDFButton.tsx``:
...
- [x] 3.1.3 The "Generating PDF…" Toast is dismissible. If the user dismisses + navigates away, the render continues. Returning to the page + clicking Export sees the cached state.

### 3.2 Next.js route updates

- [x] 3.2.1 ``frontend/src/app/api/export/pdf/[analysisId]/route.ts``: change ``GET`` handler → ``POST`` handler. Forward to the new backend ``POST /api/export/pdf/<id>`` endpoint. Return the backend's response verbatim.
- [x] 3.2.2 Add a sibling ``frontend/src/app/api/export/pdf/[analysisId]/status/route.ts``: ``GET`` forwarded to backend ``GET /api/export/pdf/<id>/status``.
- [x] 3.2.3 Authentication / cookie forwarding: reuse ``backendFetch`` so the Cognito JWT is attached.

### 3.3 Frontend tests

- [x] 3.3.1 ``ExportPDFButton.test.tsx``: cover the four UX flows:
...
- [x] 3.3.2 Mock ``fetch`` for the polling cycle; assert poll cadence matches 2 s × 10 then 5 s back-off.

### 3.4 Ship Phase 3

- [x] 3.4.1 Architecture-reviewer pass.
- [x] 3.4.2 PR + dev deploy. Verify the new flow works end-to-end on dev with a real analysis.
- [x] 3.4.3 Dev soak: ~24 hours, no user-visible regressions.
- [x] 3.4.4 Promote dev → testing. Soak ~24 hours.
- [x] 3.4.5 Promote testing → production.
...
- [x] 4.2.1 PR + dev deploy. CI green.
- [x] 4.2.2 Promote dev → testing → production.
- [x] 4.2.3 Final sanity: a fresh production analysis still exports correctly.

## 5. Post-prod soak

### 5.1 Telemetry

- [x] 5.1.1 Check Lambda invocation counts: cached-path POSTs should NOT trigger PDF Lambda invocations. CloudWatch ``Invocations`` metric drops materially for the PDF Lambda.
- [x] 5.1.2 Check S3 ``BucketSizeBytes`` growing slowly as new analyses cache.
- [ ] 5.1.3 No DLQ messages on the PDF Lambda async-invoke for ~7 days. _(Open soak item — production clean since 2026-05-18 deploy; CloudWatch DLQ-depth alarm covers ongoing.)_
- [ ] 5.1.4 No HTTP 504s on the export endpoint for ~7 days. _(Open soak item — production clean since 2026-05-18 deploy.)_

### 5.2 Quality monitoring

- [x] 5.2.1 Spot-check 5 production PDFs (cached + cold paths). Same quality bar as pre-migration.
- [x] 5.2.2 No user reports of "PDF download started something different" (e.g., wrong analysis).

## 6. Wrap-up

- [x] 6.1 Sync delta spec into ``openspec/specs/polished-pdf-export/spec.md``.
