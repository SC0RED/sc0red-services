# mcp-connect-onboarding Specification

## Purpose
The self-serve Connect page: it presents the customer's per-environment MCP server address, per-client OAuth setup instructions (Claude Desktop + Cursor full walkthroughs, ChatGPT a hedged pointer), and in-page management of connected assistants (OAuth-consent list + disconnect). OAuth-only — there is no API-key path. (Where the page lives in the app is owned by the `authenticated-layout` capability, not here.)
