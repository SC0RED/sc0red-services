# domain-tooltips Specification

## Purpose

Inline help tooltips for PE-domain terms (Risk Tier, Risk Score, EBITDA Tree, Value Lever, Active Lever Filter, Industry, Impact Rating). Content lives in a single shared registry, so the same term explained on `/analyses` and `/analysis/{id}` reads identically. Tooltips are keyboard- and screen-reader-accessible via `role="tooltip"` + `aria-describedby`.
