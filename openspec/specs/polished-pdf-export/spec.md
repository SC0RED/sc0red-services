...
The export endpoint SHALL be `POST /api/export/pdf/<analysisId>` (the prior synchronous `GET` route was retired in the `async-pdf-export-with-cache` change). The endpoint behaviour depends on the analysis's `pdfExport` sub-record:
...
- **THEN** they are stripped or replaced; the filename is at most 80 characters; the presigned URL's `ResponseContentDisposition` carries the sanitised filename so the browser saves it correctly
...
A dedicated S3 bucket `janus-<env>-pdf-exports` SHALL store rendered PDFs. Objects SHALL be keyed by `pdf-exports/<analysisId>.pdf` — one object per analysis. Re-renders overwrite the previous object.
...
1. Render the PDF (Puppeteer + headless Chromium against the print route).
...
The PDF render Lambda SHALL have a DLQ configured for async-invoke failures (Lambda's retry budget exhausted). A CloudWatch alarm SHALL fire on any DLQ message in monitored environments.
