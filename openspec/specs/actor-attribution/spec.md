...
- **THEN** `validate_token` raises `ValueError`
...
This requires a Global Secondary Index keyed on `cognito_sub` (GSI5 in the Janus single-table design). The index MUST be deployed and reach `ACTIVE` status before any consumer of `find_by_cognito_sub` ships, otherwise the lookup either returns `None` for every existing record (sparse-index reality on day 0) or fails with `ValidationException`.
...
### Requirement: User records carry GSI1PK so find_by_org returns them

Every user record SHALL carry `GSI1PK = "ORG#{org_id}"` and `GSI1SK = "USER#{user_id}"` so that `user_repository.find_by_org(org_id)` returns it. Records written before this requirement was enforced (legacy records lacking `GSI1PK`) SHALL be backfilled by a one-shot script that uses table-scan-based discovery (not `find_by_org`, which has a chicken-and-egg problem on those exact records) and writes the missing GSI1 keys.

This was the acute root cause of the actor-attribution bug surfaced on PR #215's smoke test: without `GSI1PK`, `find_by_org` returned empty, the `_resolve_actor` lookup map was empty, and every actor name fell through to the placeholder fallback regardless of whether the stored id was a Cognito sub or an internal user id.

#### Scenario: Legacy user record lacks GSI1PK

- **WHEN** a user record was created before `user_repository.create()` started writing `GSI1PK`
- **AND** the record has `org_id` populated but `GSI1PK` absent
- **AND** `find_by_org(org_id)` is called
- **THEN** the record is NOT returned (the bug)
- **AND** the post-deploy backfill script must write `GSI1PK` to recover

#### Scenario: After backfill, find_by_org returns the legacy record

- **WHEN** `backfill_user_cognito_keys.py --apply` has run on a legacy record
- **AND** `find_by_org(org_id)` is called
