# strategy-map-balanced-scorecard-layout Specification

## Purpose

Page-level layout of the strategy-map section in the Balanced Scorecard format. Owns the Mission + Vision header (collapsing the previous four-row stack of Mission / Value Proposition / Strategic Priorities / Core Values into two lines + an expandable below the table), the responsive CSS-grid table layout (single column below 900 px viewport), the Core Values strip at the bottom of the table, and the Value Proposition + Strategic Priorities expandable section placed below the table.

The underlying objective rendering (cell content, hover wiring, opportunity dot strip) is owned by the `strategy-map` capability spec — this one is narrowly about page composition.

Introduced by the `redesign-analysis-visuals` change (Diagnostic Tool Feedback items #4 and #5e).
