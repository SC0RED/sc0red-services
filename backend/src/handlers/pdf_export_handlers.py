...
from src.handlers.pdf_export_storage import (
...
from src.handlers.pdf_token_secret import read_pdf_token_secret
...
# Re-exported so callers that import this module continue to see it.
__all__ = ["delete_cached_pdf"]

logger = logging.getLogger(__name__)

# A ``rendering`` sub-record older than this window is treated as stale.
# 60 s comfortably exceeds the typical render time (~13 s after the
# 2026-05-18 memory bump).
...
# Module-level Lambda client survives warm-restart, avoiding the ~50 ms
# per-invocation init. S3 + Secrets Manager clients live in their
# sibling modules (pdf_export_storage, pdf_token_secret).
...
def handle_post_export(
    ...
    if record and record["status"] == "ready":
        filename = build_download_filename(company)
        url = mint_pdf_url(bucket, record["s3_key"], filename)
