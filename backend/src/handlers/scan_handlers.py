...
def handle_scan_start(
    ...
    if not url or not scan_type:
        return error_response("url and type required")
...
def handle_scan_confirm(
    ...
    if not companies:
        return error_response("No companies provided")
    ...
    if not valid_companies:
        return error_response("At least one company with a url is required")
    ...
    return json_response({"ok": True, "queued": queued}, 202)
