...
interface Company { name: string; url: string; description: string; selected: boolean }
...
function NewScanContent() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (!res.ok) { setError(data.error || 'Analysis failed. Please try again.'); setPhase('input'); return }
            ...
            if (data.status === 'awaiting_confirmation' && data.portfolioCompanies) {
                const companiesWithSelect = data.portfolioCompanies.map((c: any) => ({ ...c, selected: true }))
    ...
    async function confirmPortfolio() {
        ...
        const progressInterval = setInterval(() => {
            ...
            setProgressLabel(`Analyzing companies... (${Math.min(completed, selected.length)}/${selected.length} complete)`)
        }, 5000)
    ...
    async function pollRunning(id: string, total: number) {
        const interval = setInterval(async () => {
            ...
            if (!res.ok) return
            const data = await res.json()
            const done = data.analyses?.filter((a: any) => a.analyzed_at)?.length || 0
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh' }}>
            <DashboardSidebar />
            <main style={{ flex: 1, marginLeft: 'var(--sidebar-width)', padding: '2.5rem', display: 'flex', justifyContent: 'center' }}>
                <div style={{ width: '100%', maxWidth: '680px' }}>

                    {/* Header */}
                    <div style={{ marginBottom: '2rem' }}>
                        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.25rem' }}>New AI Risk Scan</h1>
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            border: mode === opt.value ? '2px solid var(--accent-blue)' : '1px solid var(--border)',
                                    ...
                                    >
                                        ...
                                        <div style={{ fontWeight: 600, marginBottom: '0.25rem', color: mode === opt.value ? 'var(--text-primary)' : 'var(--text-secondary)', fontSize: '0.9375rem' }}>{opt.label}</div>
                            ...
                            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                <div className="input-group">
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <span style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-tertiary)' }}>
                                ...
                                {error && (
                                    <div style={{ padding: '0.75rem 1rem', background: 'var(--risk-critical-bg)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 'var(--radius-md)', color: 'var(--risk-critical)', fontSize: '0.875rem' }}>
                                        {error}
                                    </div>
                                )}
                    ...
                    {(phase === 'analyzing' || phase === 'running') && (
                        <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
                            <div style={{
                                ...
                                borderRadius: '50%',
                                background: 'conic-gradient(var(--accent-blue) 0%, var(--accent-cyan) 50%, var(--bg-surface-3) 50%)',
                            ...
                            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', fontSize: '0.9375rem' }}>{progressLabel}</p>
                            <div className="progress-bar" style={{ maxWidth: '360px', margin: '0 auto' }}>
                                <div className="progress-fill" style={{ width: `${progress}%` }} />
                            </div>
                            <div style={{ marginTop: '0.75rem', fontSize: '0.8125rem', color: 'var(--text-tertiary)' }}>{progress}% complete</div>
                    ...
                    {phase === 'portfolio_confirm' && (
                        <div>
                            <div className="card" style={{ padding: '1.25rem 1.5rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div style={{ width: '40px', height: '40px', borderRadius: 'var(--radius-md)', background: 'var(--risk-low-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
                                        Found {companies.length} companies. Review and deselect any you don't want to analyze.
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem', maxHeight: '400px', overflowY: 'auto' }}>
                                {companies.map((company, i) => (
                                    <div key={i} className="card-surface-2" style={{ padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
                                        <input
                                            type="checkbox"
                                            id={`company-${i}`}
                                            checked={company.selected}
                                            onChange={e => setCompanies(prev => prev.map((c, idx) => idx === i ? { ...c, selected: e.target.checked } : c))}
                                        ...
                                        <label htmlFor={`company-${i}`} style={{ flex: 1, cursor: 'pointer' }}>
                                            ...
                                            <div style={{ color: 'var(--text-tertiary)', fontSize: '0.8125rem' }}>{company.url}</div>
                                        </label>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                                ...
                                <span style={{ color: 'var(--text-tertiary)', fontSize: '0.8125rem', marginLeft: 'auto' }}>
        ...
        <SessionWrapper>
            <Suspense fallback={
                <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: '32px', height: '32px', border: '3px solid var(--border)', borderTopColor: 'var(--accent-blue)', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                </div>
            }>
                <NewScanContent />
            </Suspense>
