    ...
    return (
        <div style={{ minHeight: '100vh' }}>
            ...
            <nav style={{
                ...
                background: 'rgba(6,10,18,0.8)', backdropFilter: 'blur(16px)',
            ...
            <section style={{
                ...
                textAlign: 'center', padding: '140px 2rem 100px',
                position: 'relative', overflow: 'hidden',
            }} className="grid-bg">
                <div style={{
                    position: 'absolute', inset: 0, zIndex: 0,
                    background: 'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(59,123,246,0.12) 0%, transparent 70%)',
                }} />
                <div style={{ position: 'relative', zIndex: 1, maxWidth: '860px', margin: '0 auto' }}>
                    <div className="badge badge-blue" style={{ marginBottom: '1.5rem', fontSize: '0.8125rem' }}>
                        <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--accent-blue)', display: 'inline-block', animation: 'pulse 2s infinite' }} />
                    ...
                    <p style={{ fontSize: '1.25rem', color: 'var(--text-secondary)', maxWidth: '600px', margin: '0 auto 2.5rem', lineHeight: 1.7 }}>
                        Janus analyzes your portfolio companies for AI-driven disruption risks and generates specific, tactical roadmaps to defend and grow.
                    </p>
            ...
            <section style={{ padding: '0 2rem 6rem', maxWidth: '1100px', margin: '0 auto' }}>
                <div className="card" style={{ overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                            {['var(--risk-critical)', 'var(--risk-moderate)', 'var(--risk-low)'].map((c, i) => (
                                <div key={i} style={{ width: '12px', height: '12px', borderRadius: '50%', background: c, opacity: 0.7 }} />
                            ))}
                        </div>
                        <span style={{ fontSize: '0.8125rem', color: 'var(--text-tertiary)' }}>Janus Analysis — Acme Financial Services</span>
                    </div>
                    <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: '200px 1fr', gap: '1.5rem' }}>
                        <div style={{ textAlign: 'center', padding: '1.5rem', background: 'var(--bg-surface-2)', borderRadius: 'var(--radius-md)' }}>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Risk Score</div>
                            <div style={{ fontSize: '3.5rem', fontWeight: 800, color: 'var(--risk-high)', lineHeight: 1 }}>7.4</div>
                            <div className="badge badge-high" style={{ marginTop: '0.75rem' }}>High Risk</div>
                        </div>
                        <div>
                            <div style={{ marginBottom: '0.75rem', fontWeight: 600, fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Risk Assessment</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {risks.map(r => (
                                    <div key={r.name} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
                                        <div style={{ width: '140px', fontSize: '0.8rem', color: 'var(--text-secondary)', flexShrink: 0 }}>{r.name}</div>
                                        <div style={{ flex: 1, height: '6px', background: 'var(--bg-surface-3)', borderRadius: '3px', overflow: 'hidden' }}>
                                            <div style={{ height: '100%', width: `${(r.score / 10) * 100}%`, background: tierColors[r.tier], borderRadius: '3px' }} />
                                        </div>
                                        <div style={{ width: '32px', fontSize: '0.8rem', fontWeight: 700, color: tierColors[r.tier], textAlign: 'right' }}>{r.score}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
                    {features.map(f => (
                        <div key={f.title} className="card" style={{ padding: '1.75rem' }}>
                            <div style={{ width: '48px', height: '48px', borderRadius: 'var(--radius-md)', background: `${f.color}18`, border: `1px solid ${f.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: f.color, marginBottom: '1.25rem' }}>
                                {f.icon}
                            </div>
                            <h3 style={{ fontWeight: 700, marginBottom: '0.625rem', fontSize: '1.0625rem' }}>{f.title}</h3>
            ...
            <section style={{ padding: '4rem 2rem 6rem', textAlign: 'center' }}>
                <div className="card" style={{ maxWidth: '600px', margin: '0 auto', padding: '3rem', borderColor: 'rgba(59,123,246,0.2)' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', lineHeight: 1.7 }}>
                        Join PE firms and operators using Janus to identify AI threats and build defensible, AI-native business strategies.
                    </p>
                    <Link href="/signup" className="btn btn-primary btn-lg" style={{ display: 'inline-flex' }}>
            ...
            {/* Footer */}
            <footer style={{ padding: '2rem', borderTop: '1px solid var(--border-subtle)', textAlign: 'center' }}>
